...
In particular, WRF exaggerates the diurnal range of temperatures, incorrectly estimates the timing and intensity of $\Delta T$, and over-estimates the daytime relationship between vegetation and temperature. 
...
In order to understand what causes the differences in temperature between each land type, we examine the surface energy budget in each land type. 
 ...
 To understand the causes of this forcing, we break down the surface energy budget into component parts. First, we examine net radiation fluxes (Fig.~\ref{fig:seb}b). These fluxes differ little by land type. 
...
Similarly, emissivity is a function of land type (Tab.~\ref{tab:lcc}), driving a difference in net longwave radiation between land types. 
...
A key difference between ground heat flux and the turbulent fluxes (LE + H) is that the atmosphere can carry away energy lost as LE or H.
...
The WRF-Urban model couples an urban canopy model (UCM) with the Noah land surface model to compute the surface fluxes. At each grid cell, the UCM computes fluxes from anthropogenic surfaces, which are combined from fluxes from natural surfaces by the land surface model.
...
Each model is the same for each land type, but the parameters determining the urban area differ. 
That is, the parameters $R, RW, W$ change by land type. 
These parameters are calculated from values in Table~\ref{tab:urban_model}, which shows the parameters used in the urban canopy model that vary by land type. 
...
We also find a relationship between temperature and distance to the nearest park, though not during the day.
