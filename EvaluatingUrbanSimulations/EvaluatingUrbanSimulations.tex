...
\title{Evaluating Urban Simulations with a Dense Sensor Network in Baltimore, Maryland}
...
\begin{abstract}
The health burden of heat exposure makes understanding urban climate a priority for scientists and policymakers alike, but knowledge gaps remain. 
This study addresses two active research trends in the study of urban climate: the proliferation of high-density, low-cost in situ monitoring networks, and the application of numerical weather models like the Weather Research and Forecasting (WRF) model to study phenomenology and process in urban climate. Here we apply both a monitoring network and WRF to study the urban heat island (UHI) of Baltimore, Maryland. 
A central finding is that first order metrics often applied to assess performance of mesoscale climate models in the study of the UHI are inadequate when evaluating the model’s ability to represent the physical drivers of the UHI. This is a critical issue, as the assumption that models are capable of simulating UHI process underlies confidence in their application to study urban climate vulnerabilities and UHI intervention strategies. 
A second finding is that applications of high density measurement networks to study the UHI are sensitive to sensor siting and distribution decisions relative to the land cover and location of both urban and rural monitoring sites. 
Combining these two findings, we see that the simplification of land surface classes and properties in a model like WRF complicates the evaluation and application of the model in urban applications. While these challenges are generally acknowledged in model-based studies of urban climate, the findings of this study suggest that they can drive potentially erroneous conclusions regarding UHI process and mitigation potential. As work on urban modeling and monitoring continues to advance, careful attention to the representation of both urban and neighboring rural surface properties must be taken into account when designing and interpreting high resolution urban studies.
...
%We then evaluate urban the integrated urban Weather, Research and Modeling System (WRF), a numerical model,  against these identified characteristics.
...
%We also see that urban-rural temperature differences $\Delta T$ are lowest during the day and highest at night for rural open sites, and highest during the daytime for rural forested sites, demonstrating that in addition to being sensitive to urban site characteristics, the timing and magnitude of these differences are highly sensitive to rural station micro-climate. 
% While summary statistics suggest good model performance, a more detailed evaluation shows that WRF fails to adequately capture several important UHI properties in Baltimore. 
...
\includegraphics[width = \textwidth]{../figures/figure01map.eps}
...
iButtons were installed in 91 sites beginning in May 2016, with the full network installed by July 1, 2016; at the end of the summer, 85 sensors remained. Thus, in this study we examine data taken between July 1, 2016 and August 30, 2016. Nearly all sites (79) were located within Baltimore City; the remaining 6 were north of the city in Baltimore County. During installation, local site data was taken on the surrounding site characteristics: 24 sensors were located in locations dominated by impervious surfaces, 43 were located in locations with grass and low vegetation, 13 sensors with bare or little ground-cover, and 5 sites with a mix of these characteristics. 
 ...
 \begin{figure}
\centering
\includegraphics[width=20pc]{../figures/figure02.pdf}
...
\begin{figure}
\centering
\includegraphics[width = \textwidth]{../figures/ibuttonbias.eps}
...
\begin{figure}
\centering
\includegraphics[width = .75\textwidth]{../figures/semivariogram.eps}
...
\begin{figure}
\centering
\includegraphics[width = \textwidth]{../figures/vegetation_fraction.eps}
...
\begin{figure}
\centering
\includegraphics[width = \textwidth]{../figures/veg_elevation_corrected.eps}
\caption{Surface vegetation versus temperature for observations (left column) and model (right column) for all hours (top row), 6am (middle row), 4pm (top row): in a,c, and e, satellite NDVI versus mean observed temperature at each site, and b,d, and f, 0.01 times model vegetative fraction. The correlation $r$ and correlation p-value $p$ are labeled. Colors indicate each land type. N indicates the number of model grid points included in each land type.}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{../figures/distance_to_park.eps}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../figures/DT_sensitivity.eps}
...
\subsubsection{Vegetation}
Observed temperature shows a negative correlation with satellite-measured vegetation NDVI. In the model, vegetation is represented by a parameter called vegetative fraction (VF) which varies between zero and one and comes from.% [insert details from Ben/Hamada here]. 
...
\caption{As in Fig. \ref{fig:diurnal}c,d but for the whole model domain.}% (a) Average summer temperature and (b) temperature difference by hour for for each landcover type.}% N indicates the number of model grid points included in each land type. }
...
%\caption{Average hourly surface energy flux by land type: a) net flux $R_{net} -LH + H- G$, b) net radiation $R_{net}$ (down minus up), c) sensible heat $HFX$, d) latent heat $LH$, and e) ground flux $GRDFLX$. Error bars represent spatial variability for time-mean hourly values.}
\label{fig:seb}
\end{figure}
...
%A higher energy flux for more developed land types is consistent with warmer developed land and a pronounced UHI. 
 To understand the causes of this forcing, we break down the surface energy budget into component parts. First, we examine the net radiation fluxes in Fig.~\ref{fig:seb}a. These fluxes differ little by land type, and we can attribute these differences to albedo and emissivity, which are assigned to each land type (see Table~\ref{tab:lcc}). 
...
%\centering
%\includegraphics[width=40pc]{../figures/whole_domain/diurnal_model.eps}
%\caption{As in Fig.~\ref{fig:diurnal} but for the whole model domain. (a) Average summer temperature and (b) temperature difference by hour for for each landcover type.}% N indicates the number of model grid points included in each land type. }
%\label{fig:diurnal_wd}
%\end{figure}
