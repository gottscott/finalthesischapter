...
While some studies use an extensive network to evaluate surface charactistics-for example, X used a network of 60 stations in China---very few studies evaluate the model at the resolution at which it is run, potentially affecting conclusions of model accuracy. Additionally, while all of these studies examined temperature in urban areas, only 13 explicitly examined urban-rural differences. Of these, even fewer used urban-rural differences as an evaluation metric. For example, \citet{li2013synergistic} examined how the urban heat island evolved during a heatwave using simulations, and evaluated the model using X statistic from observations at one surface observation site.  Other UHI characteristics, such as the spatial variability of temperature, or the relationship between green infrastructure and temperature, were not used to evaluate simulations. 
...
\end{equation}
where the net radiative forcing $$r_{net}=LWDNB + SWNDB-SWUPB$$. 
...
We have examined Baltimore's urban heat island using a dense network of thermometers and seen that temperature's spatial variability is around $1^\circ$C, though significant differences in temperature are attributed to land use. We find high intensity land to be the hottest and rural forest land the coolest. We find that temperature exhibits spatial auto-correlation that increases linearly with distance, and doesn't appear to reach a maximum within the distances examined (25 km). Urban-rural temperature differences are highest in the morning and are lower throughout the day, with higher intensity land exhibiting higher values of $\DeltaT$. Cooler temperatures are linked with satellite-derived vegetation in the morning, but shows a weak relationship with cooler afternoon temperatures.  
